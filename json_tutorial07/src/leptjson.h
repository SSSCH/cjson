...
typedef struct LeptJsonMember LeptJsonMember;
//Josn 结果的容器
...
struct LeptJsonMember {
    char* k;size_t klen;  //key and keylenth
    LeptJsonResult kvalue;  //key value
};
//解析完成后返回值类型
typedef enum{
    LEPT_PARSE_OK = 0,
    LEPt_PASE_STRINGFY_OK,
    LEPT_PARSE_NO_VALUE,
...
//@josnResult：josn格式的输入， @ResultString:output, @len：output,josn len
