...
//string0->json, josn->string1, string0:string1 ?
#define EXPECT_EQ_ROUNDPARSE(json) \
        do{ \
            char* s; \
            size_t len = 0; \
            LeptJsonResult leptJsonResult; \
            LEPT_TYPE_INIT(&leptJsonResult); \
            EXPECT_EQ_actual(LEPT_PARSE_OK, LeptJson_Parse(&leptJsonResult, json), "&d"); \
            EXPECT_EQ_actual(LEPt_PASE_STRINGFY_OK, lept_stringfy(&leptJsonResult, &s, &len), "%d"); \
...
static void test_parse_object(){
            LeptJsonResult leptJsonResult;
            LEPT_TYPE_INIT(&leptJsonResult);
            EXPECT_EQ_actual(LEPT_PARSE_OK, LeptJson_Parse(&leptJsonResult, "{\"true\":1}" ), "%d");
            EXPECT_EQ_actual(LEPT_OBJECT, GetParseResult(&leptJsonResult), "%d");
